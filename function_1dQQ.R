...
#Quantile-Quantile version that allow to correct Mp in CC context (out-of-sample values) (Deque, 2007; Gudmundsson et al., 2012)
...
QQb_new_for_MRec<-function(Rc,Mc,Mp,p=0){ #the parameter p is only necessary to implement QQ for MRec (see Pegram and Bardossy, 2012): p=0 classical QQ
  N=ncol(Mc)
  I_Mc=nrow(Mc)
  I_Mp=nrow(Mp)
  if(is.null(N)&is.null(I_Mc)){ #for 1d vectors
